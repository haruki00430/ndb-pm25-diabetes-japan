...
Ambient fine particulate matter (PM2.[@Sun2010AirPollutionInsulinResistance] has been implicated in cardiometabolic risk through systemic inflammation, oxidative stress, and insulin resistance, but evidence from Japan remains limited, and the magnitude of association may depend on exposure assessment and analytic scale.
...
We conducted a nationwide cross-sectional ecological study using 47 Japanese prefectures as the unit of analysis. Ambient PM2.5 exposure was summarized as the two-year mean (2022–[] of monitoring-station annual averages within each prefecture. Diabetes indicators were derived from the 10th National Database (NDB) Open Data, including prefecture-level mean HbA1c among specific health checkup participants (fiscal year [] and diabetes medication prescriptions per 100,000 population (fiscal year 2023; therapeutic category []. We integrated covariates (aging rate, obesity rate, exercise habit rate, and gross domestic product per capita) and fitted ordinary least squares (OLS) regression models. Residual normality was assessed using Shapiro–Wilk tests, and spatial autocorrelation was evaluated using Moran’s *I* with queen contiguity weights. Sensitivity analyses included restriction to prefectures with ≥10 monitoring stations, outlier exclusion using Cook’s distance (threshold 4/*N*), and stratified analyses by urbanicity. Post-hoc statistical power was estimated based on observed model *R*².

## Results

Across prefectures (*N* = [], the mean PM2.5 concentration was 8.55 ± 1.12 μg/m³ and the mean HbA1c was 5.74 ± 0.04%. PM2.5 was not significantly associated with mean HbA1c (β = 0.00371 ± 0.00529; *p* = 0.[] nor diabetes medication prescriptions per 100,000 (β = 6,884 ± 19,001; *p* = 0.[] in covariate-adjusted models (*R*² = 0.288 and 0.291, respectively). No significant residual spatial autocorrelation was detected (Moran’s *I* *p* ≥ 0.[@Sun2010AirPollutionInsulinResistance]. Null associations persisted across all sensitivity analyses.
...
Diabetes mellitus is a major public health concern in Japan, with increasing prevalence in the context of rapid population aging. Ambient air pollution—particularly fine particulate matter (PM2.[@Sun2010AirPollutionInsulinResistance]—has been proposed as a modifiable environmental risk factor for metabolic dysfunction through mechanisms including systemic inflammation, oxidative stress, endothelial dysfunction, and insulin resistance [@Yang2020AirPollutionDiabetes],[@Brook2010ParticulateMatterCVD],[@Rajagopalan2018AirPollutionCVD],[@Rajagopalan2019AirPollutionMetabolic],[@Sun2010AirPollutionInsulinResistance]. However, evidence is heterogeneous across settings [@Eze2015AirPollutionDiabetes],[@Janghorbani2014AirPollutionDiabetes],[@Weber2018PM25DiabetesGermany],[@Bowe2018PM25DiabetesCKD], and ecological analyses using nationally comprehensive administrative data can help characterize geographic patterns and generate hypotheses for future individual-level studies.
...
We conducted a cross-sectional ecological study using Japanese prefectures (*N* = [] as the unit of analysis.

## 2. Exposure: Ambient PM2.5

Ambient PM2.5 exposure was derived from national air pollution monitoring data and summarized as the two-year mean (2022–[] of monitoring-station annual averages within each prefecture [@NIES_PM25_Monitoring].

## 3. Outcomes: Diabetes-Related Indicators

### 3.1. Mean HbA1c

Prefecture-level mean HbA1c (%) was derived from the 10th National Database (NDB) Open Data specific health checkups examination tables (fiscal year [] [@MHLW_NDB_OpenData_No10]. HbA1c was calculated as a weighted average from stratum counts using prespecified stratum midpoints.

### 3.2. Diabetes Medication Prescriptions

Diabetes medication prescription volume was extracted from the 10th NDB Open Data prescription tables (fiscal year [] for therapeutic category 396 and standardized per 100,000 population [@MHLW_NDB_OpenData_No10].
...
Post-hoc power was estimated using the observed model *R*² with *N* = 47 and *k* = 5 predictors. Achieved power was 90.3% for the HbA1c model and 90.7% for the medication model. To achieve 80% power for a medium effect size (*f*² = 0.[@Coogan2012TrafficPMDiabetes], approximately *N* = 92 prefectures would be required, which exceeds the number of Japanese prefectures.
...
Across 47 prefectures, the mean (SD) PM2.5 concentration was 8.55 (1.[@Nagashima2021AirPollutionJapanReview] μg/m³ (range: 6.32–11.[@Peng2016AirPollutionDiabetesChina]. Mean (SD) HbA1c was 5.74 (0.[@Rajagopalan2019AirPollutionMetabolic]% (range: 5.65–5.[]. Mean (SD) diabetes medication prescriptions were 603,930 (154,[] per 100,000 population (range: 157,969–[] (Table [@Yang2020AirPollutionDiabetes].

## 2. Main Regression Results

In covariate-adjusted OLS models, PM2.5 was not significantly associated with either outcome. For mean HbA1c, the PM2.5 coefficient was 0.00371 (SE 0.00529; *p* = 0.487; *R*² = 0.288; adjusted *R*² = 0.[]. For medication prescriptions per 100,000 population, the PM2.5 coefficient was 6,884 (SE 19,001; *p* = 0.719; *R*² = 0.291; adjusted *R*² = 0.[].
...
In this nationwide prefecture-level ecological study, ambient PM2.5 exposure was not significantly associated with mean HbA1c nor diabetes medication prescription volume after covariate adjustment, despite substantial between-prefecture variation in diabetes-related indicators and known cardiometabolic effects of air pollution [@Brook2010ParticulateMatterCVD],[@Rajagopalan2019AirPollutionMetabolic]. Null associations were consistent across multiple sensitivity analyses and persisted in models accounting for monitor density and potential outliers, suggesting that large, easily detectable effects of PM2.5 on these aggregate outcomes are unlikely under current exposure levels in Japan [@Nagashima2021AirPollutionJapanReview].

## 2. Comparison with Prior Studies

Our null findings contrast with multiple international cohort studies and meta-analyses reporting positive associations between long-term PM2.5 exposure and type 2 diabetes incidence or glycemic deterioration, typically on the order of a few percent increase in risk per 1 μg/m³ increase in PM2.5 [@Yang2020AirPollutionDiabetes],[@Eze2015AirPollutionDiabetes],[@Janghorbani2014AirPollutionDiabetes],[@Liu2019PM25T2D_Taiwan],[@Elliott2013TrafficPMDiabetes],[@Coogan2012TrafficPMDiabetes],[@Kim2019PM25DiabetesKorea],[@Peng2016AirPollutionDiabetesChina],[@Yang2018PM25MetabolicSyndrome]. Large Asian cohorts have likewise documented elevated diabetes or metabolic risk with increasing fine particulate pollution [@Liu2019PM25T2D_Taiwan],[@Peng2016AirPollutionDiabetesChina],[@Wang2014PM25InsulinResistance], whereas several European and North American cohorts and ecological studies have reported null or borderline associations, particularly at lower exposure ranges or in highly adjusted models [@Weber2018PM25DiabetesGermany],[@Bowe2018PM25DiabetesCKD]. Within Japan, individualレベルおよび地域レベルの研究では、都市部での糖尿病有病率や妊娠糖尿病リスクとPM2.5との正の関連を示す報告 [@Mitsuhashi2019PM25DiabetesOkayama],[@Ito2023PM25GestationalDiabetesJapan] と、低濃度・限定的な地理的変動のもとで統計学的に有意な関連が検出されない報告の両方が存在する [@Nagashima2021AirPollutionJapanReview]. Taken together, the broader literature indicates that PM2.5 is biologically capable of impairing glucose metabolism through pathways such as systemic inflammation, oxidative stress, endothelial dysfunction, and adipose tissue inflammation [@Rajagopalan2019AirPollutionMetabolic],[@Sun2010AirPollutionInsulinResistance], but the detectability and magnitude of epidemiologic associations appear highly sensitive to exposure range, study design, outcome definition, and spatial scale.

## 3. Why Might No Association Be Observed?

Several methodological and data-related factors likely contributed to the absence of detectable associations in this prefecture-level analysis. First, ecological fallacy and within-prefecture heterogeneity are intrinsic to using large administrative units: prefectural averages obscure within-area contrasts between highly polluted transport corridors and cleaner rural zones, and aggregate covariates cannot capture individual-level risk factors such as diet, socioeconomic status, or family history [@Morgenstern1995EcologicStudies],[@Greenland1994EcologicBias]. Second, the use of monitoring-station means as exposure proxies introduces spatial misalignment and classical/Berkson measurement error, which tends to attenuate concentration–response relationships toward the null, particularly when the number and spatial distribution of monitors are uneven [@Zeger2000MeasurementError]. Third, the NDB Open Data outcomes themselves are structurally constrained—drug claims are provided as prescription volume rather than patient counts, and HbA1c data are limited to specific health checkup participants aged 40–74 years—raising concerns about selection bias and imperfect correspondence with true diabetes prevalence or control [@MHLW_NDB_OpenData_No10]. Fourth, strong correlations among prefecture-level covariates (aging, obesity, health behavior, economic indicators) generate multicollinearity, inflating standard errors and reducing the precision of PM2.5 effect estimates, a challenge frequently noted in ecological and spatial regression analyses of chronic disease risk [@Morgenstern1995EcologicStudies],[@Anselin1988SpatialEconometrics]. Finally, with only 47 observations, the study is well powered to detect large overall model effects but underpowered for small partial effects of a single exposure after adjustment for multiple correlated confounders. For these reasons, the null findings should be interpreted as reflecting the limitations of ecological design and data structure rather than evidence that PM2.5 is metabolically harmless.

## 3.1. Robustness (Sensitivity Analyses)

Null associations were robust across three sensitivity analyses, which targeted key sources of potential bias identified in ecological and spatial epidemiology [@Morgenstern1995EcologicStudies],[@Anselin1988SpatialEconometrics]. Restricting to prefectures with ≥10 monitoring stations (*N* = [] yielded non-significant PM2.5 coefficients for HbA1c (β = 0.00311; *p* = 0.[] and prescriptions (β = 3,963; *p* = 0.[], suggesting that limited monitor density alone is unlikely to obscure a large association. Outlier exclusion using Cook’s distance (threshold 4/*N*) did not materially change inference (HbA1c: *N* = 43, β = 0.00467; *p* = 0.315; prescriptions: *N* = 42, β = 13,901; *p* = 0.[], and stratified analyses by urbanicity also showed no significant associations in either stratum. These patterns are consistent with prior reports in which effect estimates for PM2.5 and diabetes-related outcomes were relatively stable across reasonable modeling choices but sometimes crossed the null due to limited power and correlated covariates [@Weber2018PM25DiabetesGermany],[@Bowe2018PM25DiabetesCKD].

## 4. Strengths

Key strengths of this study include nationwide coverage of all 47 prefectures, harmonized exposure and outcome definitions based on public administrative data, and extensive sensitivity analyses addressing outliers, monitor density, and urban–rural differences [@MHLW_NDB_OpenData_No10],[@NIES_PM25_Monitoring]. The consistent lack of association across these alternative specifications, despite favorable model diagnostics for normality and residual structure, suggests that the null findings are not an artifact of a single modeling choice or isolated modeling decision. In the broader PM2.5–diabetes literature, well-conducted null or weakly positive studies remain underreported relative to those showing clear risk elevations [@Eze2015AirPollutionDiabetes],[@Janghorbani2014AirPollutionDiabetes],[@Yang2020AirPollutionDiabetes]; our results therefore provide a useful counterpoint to predominantly positive findings and help delineate the conditions under which associations are most and least likely to be detected.

## 5. Limitations

Limitations include the ecological design, potential exposure misclassification, time-window mismatch between exposure and outcomes, residual confounding, and limited ability to detect small effects given the fixed number of prefectures [@Morgenstern1995EcologicStudies],[@Greenland1994EcologicBias],[@Zeger2000MeasurementError]. The relatively narrow PM2.5 exposure range in contemporary Japan compared with heavily polluted settings further constrains the contrast available for estimating concentration–response relationships and may contribute to null or imprecise estimates even when underlying biologic effects exist [@Nagashima2021AirPollutionJapanReview],[@Liu2019PM25T2D_Taiwan].

An additional limitation is multicollinearity among prefecture-level covariates (e.g., obesity, smoking, and exercise), which may inflate standard errors and reduce the precision of partial effect estimates. We therefore report sensitivity analyses and emphasize cautious interpretation of null partial associations for PM2.5.

## 6. Implications and Future Directions

Future work should move beyond coarse prefectural averages by incorporating finer geographic units, such as municipalities or secondary medical areas, and by using high-resolution exposure models that integrate satellite-derived aerosol optical depth and land-use information to reduce spatial misalignment [@Anselin1988SpatialEconometrics],[@Zeger2000MeasurementError]. Individual-level longitudinal studies linking residential histories, clinical data, and detailed behavioral risk factors would allow more rigorous control of confounding and formal assessment of effect modification by socioeconomic status or comorbidities, building on existing cohort evidence from Asia and other regions [@Liu2019PM25T2D_Taiwan],[@Kim2019PM25DiabetesKorea],[@Peng2016AirPollutionDiabetesChina],[@Yang2018PM25MetabolicSyndrome]. In parallel, multipollutant and component-specific models focusing on toxicologically relevant PM2.5 constituents (e.g., black carbon, organic matter, secondary aerosols) may help clarify whether particular mixtures are more diabetogenic than others and better align epidemiologic contrasts with mechanistic insights from experimental studies [@Rajagopalan2019AirPollutionMetabolic],[@Sun2010AirPollutionInsulinResistance]. Finally, triangulating evidence from ecological analyses, individual-level cohorts, and mechanistic experiments will be essential to fully characterize how ambient air pollution contributes to diabetes risk and to identify the populations most likely to benefit from air quality improvements [@Yang2020AirPollutionDiabetes],[@Eze2015AirPollutionDiabetes],[@Nagashima2021AirPollutionJapanReview].

---

# Conclusion

Prefecture-level ambient PM2.5 exposure was not detectably associated with diabetes-related indicators in Japan in this ecological analysis using NDB Open Data. These null findings were robust to sensitivity analyses but require cautious interpretation given exposure assessment limitations and the constraints of ecological inference.
...
3) Rajagopalan, Sanjay, Al-Kindi, Sadeer G. et al.. Air Pollution and Cardiovascular Disease: JACC State-of-the-Art Review. Journal of the American College of Cardiology. 2018;72(17):2054--2070. doi:10.1016/j.jacc.2018.07.099

4) Rajagopalan, Sanjay and Brook, Robert D.. Ambient air pollution and metabolic disorders: From mechanisms to clinical evidence. Diabetologia. 2019;62(10):1821--1830. doi:10.1007/s00125-019-4950-2
...
12) Nagashima, Seiji, Yamagishi, Kazumasa et al.. Ambient air pollution and cardiometabolic diseases in Japan: A narrative review. Journal of Atherosclerosis and Thrombosis. 2021;28(9):839--853. doi:10.5551/jat.58534
...
23) Greenland, Sander and Robins, James. Ecologic bias, confounding, and effect modification. International Journal of Epidemiology. 1994;23(1):133--140. doi:10.1093/ije/23.1.133
